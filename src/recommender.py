...
def cafe_recommender(user_prefs_general: dict,
    ...
    # Move around the columns for correct mapping for price weights
    price_columns = ["price_point_mid", "price_point_high"]
    non_price_columns = [col for col in full_df.columns if col not in price_columns]
    full_df = full_df[non_price_columns + price_columns]

    # Assign weights; avoid multicolinearity
    mid_price_pref = user_prefs_general.get("price_point_mid")
    high_price_pref = user_prefs_general.get("price_point_high")
    # Mid is better than low is better than high
    if mid_price_pref == 1:
        price_weights = [1.0, 0.3]
    # High is better than mid is better than low
    elif high_price_pref == 1:
        price_weights = [0.7, 1.0]
    # Low price preference; Low is better than mid is better than high
    else:
        price_weights = [0.7, 0.3]

    # Switch binary with weights
    working_prefs = {k: v for k, v in user_prefs_general.items() if k not in ["price_point_mid", "price_point_high"]}
    ...
    user_general_vect = np.array(list(working_prefs.values()) + price_weights)
